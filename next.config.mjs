...
// Load the JSON file without using import assertions (avoid Node loader errors)
// import assertions (assert { type: 'json' }) can cause a SyntaxError in some
// Node/Next.js environments, so read the file at runtime instead.
const docsJsonPath = path.join(process.cwd(), 'configs', 'docs.json');
let docsData = {};
try {
  const raw = fs.readFileSync(docsJsonPath, 'utf8');
  docsData = JSON.parse(raw);
} catch (err) {
  console.error(`Failed to read configs/docs.json at ${docsJsonPath}:`, err);
}
